...
training_data_with_comp <- subset(training_data_with_comp, OComp > 0.50)

library(neuralnet)
nn2=neuralnet(Outcome~Score+TR+FGP+FGP3+FTP+Ast+TO+Stl+Blk, data=training_data_with_comp, hidden=c(6,4), act.fct="logistic", linear.output=FALSE, lifesign="full", threshold=30.0)
