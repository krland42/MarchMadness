...
regular_season_2019_winning_stats <- subset(regular_season_detailed, Season == 2019, select = c(3, 4, 9:21))
regular_season_2019_losing_stats <- subset(regular_season_detailed, Season == 2019, select = c(5, 6, 22:34))
...
detach("package:plyr", unload = TRUE)
library(dplyr)
test <- select(regular_season_2019_average_stats, Score, TR, FGP, FGP3, FTP, Ast, TO, Stl, Blk)

detach("package:dplyr", unload = TRUE)
library(neuralnet)
predict=compute(nn,test)
competitiveness <- data.frame(regular_season_2019_average_stats$TeamID, predict$net.result)
detach("package:neuralnet", unload = TRUE)
library(dplyr)
competitiveness <- rename(competitiveness, c(TeamID=regular_season_2019_average_stats.TeamID, competitiveness=predict.net.result))
...
tourney_2019_outcomes <- merge(tourney_2019_outcomes, competitiveness, by.x="LTeamID", by.y="TeamID")
...
tourney_2019_outcomes <- merge(tourney_2019_outcomes, competitiveness, by.x="WTeamID", by.y="TeamID")
